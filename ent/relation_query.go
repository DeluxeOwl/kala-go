...
func (rq *RelationQuery) QueryPermissions() *PermissionQuery {
...
func (rq *RelationQuery) WithPermissions(opts ...func(*PermissionQuery)) *RelationQuery {
	query := &PermissionQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RelationQuery) sqlAll(ctx context.Context) ([]*Relation, error) {
	...
	if query := rq.withRelTypeconfigs; query != nil {
		...
		for _, node := range nodes {
			...
			node.Edges.RelTypeconfigs = []*TypeConfig{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(relation.RelTypeconfigsPrimaryKey[0], fks...))
			},
		...
		query.Where(typeconfig.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
	...
	if query := rq.withPermissions; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(relation.PermissionsPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
