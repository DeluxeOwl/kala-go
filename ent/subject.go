...
func (e SubjectEdges) TypeOrErr() (*TypeConfig, error) {
	if e.loadedTypes[0] {
		if e.Type == nil {
...
func (*Subject) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case subject.ForeignKeys[0]: // type_config_subjects
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case subject.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field type_config_subjects", value)
			} else if value.Valid {
				s.type_config_subjects = new(int)
				*s.type_config_subjects = int(value.Int64)
			}
		}
	}
	return nil
